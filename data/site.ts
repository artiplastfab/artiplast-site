  ...
  de: {
    ...(() => {
      const en = {
        ...
        brandSubtitle: "Hersteller",
        searchPlaceholder: "Suchen...",
        navigation: { home: "Startseite", products: "Produkte", catalog: "Katalog", about: "Über uns", faq: "Info", contact: "Kontakt" },
        actions: {
...
const productSeeds: ProductSeed[] = [
  {
    ...
    translations: {
      ...
      ru: { name: "Потолочная Ламбрия", category: "Потолочные Решения", kicker: "Премиальный потолочный профиль", description: "Чистое потолочное решение для светлых интерьеров.", longDescription: "Потолочная ламбрия подготовлена для премиальных проектов с упорядоченным ритмом и выверенной поверхностью.", features: ["Упорядоченный ритм потолка", "Выразительная поверхность", "Готово к вариантам исполнения"], applications: ["Жилые потолки", "Торговые интерьеры", "Гостиничные проекты"], confidencePoints: ["Подходит для экспортной презентации", "Открыто для вариантов профиля и отделки", "Поддерживает визуальную структуру каталога", "Готово для технических листов"], alt: "Визуал потолочной ламбрии ArtıPLAST" },
    },
  },
  {
    slug: "kaval-borusu",
    ...
    translations: {
      ...
      ru: { name: "Круглая Профильная Труба", category: "Профильные Решения", kicker: "Функциональная трубчатая профильная группа", description: "Функциональный профильный продукт с простой геометрией и надежным производственным образом.", longDescription: "Круглая профильная труба является одной из сильнейших продуктовых групп благодаря технической ясности.", features: ["Четкая цилиндрическая форма", "Подходит для технических обсуждений", "Готово для данных по размерам и отделке"], applications: ["Профильные системы", "Интерьерные решения", "Проекты под заказ"], confidencePoints: ["Готово к техническому расширению", "Подходит для индивидуальных запросов", "Укрепляет образ производителя", "Поддерживает детальную подачу"], alt: "Визуал круглой профильной трубы ArtıPLAST" },
    },
  },
  {
    slug: "duvar-lambiri",
    ...
    translations: {
      ...
      ru: { name: "Стеновая Ламбрия", category: "Стеновые Решения", kicker: "Декоративная система стеновой облицовки", description: "Современное премиальное стеновое решение с выразительным характером поверхности.", longDescription: "Стеновая ламбрия разработана для интерьерных проектов, которым нужен выверенный визуальный эффект и чистый ритм.", features: ["Подходит для вертикального и горизонтального применения", "Сильный эффект для реальной фотосъемки", "Открыто для цветовых и отделочных вариантов"], applications: ["Акцентные стены", "Шоурумы", "Коммерческие интерьеры"], confidencePoints: ["Подходит для архитектурного сторителлинга", "Гибко для вариаций отделки", "Поддерживает прямой контакт", "Соответствует премиальному позиционированию"], alt: "Визуал стеновой ламбрии ArtıPLAST" },
    },
  },
];
...
const productNameOverrides: Record<Locale, Record<string, string>> = {
  ...
  ru: {
    "tavan-lambiri": "PVC",
    "kaval-borusu": "Потолочные, Дверные И Стеновые Ламбрии",
    "duvar-lambiri": "Дверные И Оконные Наличники",
  },
};
...
export function getSearchEntries(locale: Locale): SearchEntry[] {
  ...
  const coreEntries: SearchEntry[] = [
    ...
    { title: c.navigation.catalog, description: c.pages.products.description, href: localizedPath(locale, "/katalog"), keywords: ["catalog", "pdf", "download"] },
...
export function getNavigation(locale: Locale): NavItem[] {
  ...
  const aboutChildrenByLocale: Record<Locale, Array<{ label: string; href: string }>> = {
    ...
    ru: [
      { label: "Наш Завод", href: localizedPath(locale, "/about#fabrikamiz") },
      { label: "Наша Продукция", href: localizedPath(locale, "/products") },
      { label: "Наши Услуги", href: localizedPath(locale, "/about#hizmetlerimiz") },
      { label: "Почему Мы", href: localizedPath(locale, "/about#neden-biz") },
    ],
  };
...
export function getLocalizedAboutSections(locale: Locale) {
  const sections: Record<Locale, Array<{ title: string; description: string }>> = {
    tr: [
      { title: "Karaman Merkezli", description: "ArtıPLAST, Karaman'da üretim yapan ve uluslararası görünürlüğe odaklanan bir üreticidir." },
      { title: "Üretim Yaklaşımı", description: "Düzenli iletişim, temiz sunum ve ürün netliği markanın merkezindedir." },
      { title: "Görsel Güven", description: "Ürün ve fabrika görselleri için hazırlanmış premium alanlar güven algısını güçlendirir." },
    ...
    de: [
      { title: "Mit Sitz in Karaman", description: "ArtıPLAST ist ein Hersteller aus Karaman mit hochwertiger internationaler Ausrichtung." },
      { title: "Produktionsansatz", description: "Klare Kommunikation, saubere Präsentation und Produktklarheit stehen im Zentrum der Marke." },
      { title: "Visuelles Vertrauen", description: "Vorbereitete Premium-Bereiche für Produkt- und Fabrikmedien stärken das Vertrauen." },
      { title: "Unternehmerische Balance", description: "Wenig Text, starke Bilder und großzügige Abstände schaffen eine ruhige Markenwirkung." },
    ],
    ar: [
      { title: "من كارامان", description: "ArtıPLAST شركة تصنيع في كارامان تركّز على حضور دولي موثوق وعالي الجودة." },
      { title: "نهج الإنتاج", description: "التواصل الواضح والعرض النظيف ووضوح المنتج تشكل جوهر العلامة." },
      { title: "الثقة البصرية", description: "المساحات المجهزة لصور المصنع والمنتجات تعزز الثقة في العرض." },
      { title: "التوازن المؤسسي", description: "نص قليل وصور قوية ومساحات مدروسة تصنع لغة علامة هادئة وراقية." },
    ],
    az: [
      { title: "Karaman Mərkəzli", description: "ArtıPLAST Karaman əsaslı, beynəlxalq görünürlüğə yönəlmiş istehsalçıdır." },
      { title: "İstehsal Yanaşması", description: "Aydın kommunikasiya, təmiz təqdimat və məhsul dəqiqliyi brendin əsasını təşkil edir." },
      { title: "Vizual Etibar", description: "Məhsul və zavod görüntüləri üçün hazırlanmış premium sahələr etibarı gücləndirir." },
      { title: "Korporativ Balans", description: "Az mətn, güclü vizuallar və zərif boşluqlar sakit bir brend dili yaradır." },
    ],
    bg: [
      { title: "Базиран в Караман", description: "ArtıPLAST е производител от Караман с фокус върху международно присъствие и доверие." },
      { title: "Подход към производството", description: "Ясната комуникация, чистото представяне и продуктовата яснота са в основата на марката." },
      { title: "Визуално доверие", description: "Подготвените premium зони за продуктови и фабрични визии засилват доверието." },
      { title: "Корпоративен баланс", description: "Малко текст, силни визии и изчистени отстояния създават спокойна маркова среда." },
    ],
    fr: [
      { title: "Basé à Karaman", description: "ArtıPLAST est un fabricant basé à Karaman avec une présence premium tournée vers l’international." },
      { title: "Approche de production", description: "Communication claire, présentation propre et lisibilité produit définissent la marque." },
      { title: "Confiance visuelle", description: "Des espaces premium préparés pour les visuels usine et produit renforcent la confiance." },
      { title: "Équilibre corporate", description: "Peu de texte, des visuels forts et de beaux espacements créent une signature calme et haut de gamme." },
    ],
    ru: [
      { title: "Основано В Карамане", description: "ArtıPLAST — производитель из Карамана с акцентом на международное присутствие и доверие." },
      { title: "Производственный Подход", description: "Ясная коммуникация, чистая подача и понятность продукции лежат в основе бренда." },
      { title: "Визуальное Доверие", description: "Подготовленные премиальные зоны для визуалов завода и продукции усиливают доверие." },
      { title: "Корпоративный Баланс", description: "Немного текста, сильные визуалы и выверенные отступы создают спокойный премиальный язык бренда." },
    ],
  };
...
export function getLocalizedFaqItems(locale: Locale) {
  const faqItems: Record<Locale, Array<{ question: string; answer: string }>> = {
    ...
    de: [
      { question: "Welche Produkte werden auf der Website gezeigt?", answer: "Die Website präsentiert PVC, Tür Wand Deckenlambris und Tür Fensterleiste als Hauptproduktgruppen." },
      { question: "Wird dieselbe Nummer für Telefon und WhatsApp verwendet?", answer: `Ja. ${PHONE_DISPLAY} ist sowohl für Telefon als auch für WhatsApp aktiv.` },
      { question: "Ist die Adresse aktuell?", answer: ADDRESS },
      { question: "Sind alle Sprachoptionen aktiv?", answer: "Ja. Türkisch, Englisch, Deutsch, Arabisch, Aserbaidschanisch, Bulgarisch und Französisch sind verfügbar." },
    ],
    ar: [
      { question: "ما المنتجات المعروضة على الموقع؟", answer: "يعرض الموقع PVC وكسوة الأبواب والجدران والأسقف وإطار الأبواب والنوافذ كعائلات المنتج الرئيسية." },
      { question: "هل يتم استخدام نفس الرقم للهاتف وواتساب؟", answer: `نعم. الرقم ${PHONE_DISPLAY} نشط للهاتف وواتساب.` },
      { question: "هل العنوان الحالي محدث؟", answer: ADDRESS },
      { question: "هل جميع خيارات اللغة مفعلة؟", answer: "نعم. التركية والإنجليزية والألمانية والعربية والأذربيجانية والبلغارية والفرنسية متاحة." },
    ],
    az: [
      { question: "Saytda hansı məhsullar təqdim olunur?", answer: "Saytda PVC, Qapı Divar Tavan Lambirisi və Qapı Pəncərə Pərvazı əsas məhsul ailələri kimi təqdim olunur." },
      { question: "Telefon və WhatsApp üçün eyni nömrə istifadə olunur?", answer: `Bəli. ${PHONE_DISPLAY} həm telefon, həm də WhatsApp üçün aktivdir.` },
      { question: "Ünvan məlumatı aktualdır?", answer: ADDRESS },
      { question: "Bütün dil seçimləri aktivdirmi?", answer: "Bəli. Türk, İngilis, Alman, Ərəb, Azərbaycan, Bolqar və Fransız dilləri aktivdir." },
    ],
    bg: [
      { question: "Кои продукти са представени на сайта?", answer: "Сайтът представя PVC, ламперия за врати стени тавани и перваз за врати и прозорци като основни продуктови групи." },
      { question: "Един и същ номер ли се използва за телефон и WhatsApp?", answer: `Да. ${PHONE_DISPLAY} е активен както за телефон, така и за WhatsApp.` },
      { question: "Актуален ли е адресът?", answer: ADDRESS },
      { question: "Активни ли са всички езикови опции?", answer: "Да. Налични са турски, английски, немски, арабски, азербайджански, български и френски." },
    ],
    fr: [
      { question: "Quels produits sont présentés sur le site ?", answer: "Le site présente PVC, lambris porte mur plafond et moulure porte fenêtre comme familles de produits principales." },
      { question: "Le même numéro est-il utilisé pour le téléphone et WhatsApp ?", answer: `Oui. ${PHONE_DISPLAY} est actif pour le téléphone et WhatsApp.` },
      { question: "L’adresse est-elle à jour ?", answer: ADDRESS },
      { question: "Toutes les langues sont-elles actives ?", answer: "Oui. Le turc, l’anglais, l’allemand, l’arabe, l’azerbaïdjanais, le bulgare et le français sont disponibles." },
    ],
    ru: [
      { question: "Какая продукция представлена на сайте?", answer: "На сайте представлены PVC, потолочные, дверные и стеновые ламбрии, а также дверные и оконные наличники как основные продуктовые группы." },
      { question: "Один и тот же номер используется для телефона и WhatsApp?", answer: `Да. Номер ${PHONE_DISPLAY} активен и для телефона, и для WhatsApp.` },
      { question: "Актуален ли адрес?", answer: ADDRESS },
      { question: "Все языковые версии активны?", answer: "Да. Доступны турецкий, английский, немецкий, арабский, азербайджанский, болгарский, французский и русский языки." },
    ],
  };
